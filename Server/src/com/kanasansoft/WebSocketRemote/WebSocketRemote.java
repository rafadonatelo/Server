...
public class WebSocketRemote implements OnMessageObserver, OnCaptureObserver {
	...
	public WebSocketRemote() throws Exception {
		...
		/*
		 * ResourceHandler resourceHandler = new ResourceHandler(); String
		 * htmlPath =
		 * this.getClass().getClassLoader().getResource("html").toExternalForm
		 * (); resourceHandler.setResourceBase(htmlPath);
		 * 
		 * WSServlet wsServlet = new WSServlet(this); ServletHolder
		 * wsServletHolder = new ServletHolder(wsServlet);
		 * wsServletHolder.setInitParameter("bufferSize",
		 * Integer.toString(8192*256,10)); ServletContextHandler
		 * wsServletContextHandler = new ServletContextHandler();
		 * wsServletContextHandler.addServlet(wsServletHolder, "/ws/*");
		 * 
		 * HandlerList handlerList = new HandlerList();
		 * handlerList.setHandlers(new Handler[] {resourceHandler,
		 * wsServletContextHandler}); server.setHandler(handlerList);
		 * server.start();
		 * 
		 * Capture capture = new Capture(); capture.setOnCaptureObserver(this);
		 * capture.start();
		 */
	...
	@Override
	synchronized public void onMessage(byte arg0, String data) {
		System.out.println("onMessage! = " + arg0 + " + " + data);
	}
	...
	synchronized public void onMessage(Connection connection, String data) {
		...
		if (messageType.equals("image")) {
			...
			messages = data.split("\\|", 5);
			fileUploadEvent.setFileName(messages[3]);
			String content = messages[4];
			fileUploadEvent.setBase64data(content);
			fileUploadEvent.save(fileUploadEvent.getFileName());
		}
